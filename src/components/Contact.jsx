...
import { FiMail, FiPhone, FiSend, FiGithub, FiLinkedin, FiTwitter } from 'react-icons/fi';
...
export default function Contact() {
  ...
  return (
    <section id="contact" className="py-24 bg-gradient-to-b from-slate-900 to-slate-950 text-white">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="text-center">
            ...
            <p className="text-xl text-gray-300 max-w-2xl mx-auto">
              Have a project in mind or just want to say hi? Let’s connect and create something awesome together.
            </p>
          </motion.div>
            ...
            <motion.div variants={itemVariants} className="space-y-8">
              <div>
                ...
                <p className="text-gray-300 leading-relaxed mb-6">
                  I’m always open to collaborating on interesting projects. Reach out through any platform.
                </p>

                {/* Info Cards */}
                {contactInfo.map((info, idx) => (
                  <div key={idx} className="flex items-center gap-4 hover:translate-x-1 transition">
                    <div className="p-3 rounded-xl bg-white/5 border border-white/10 text-cyan-400">
                      <info.icon className="w-5 h-5" />
                    </div>
                    <div>
                      <p className="font-medium">{info.title}</p>
                      <a href={info.href} className="text-gray-300 hover:text-cyan-400 transition-colors">
                        {info.value}
                      </a>
                    </div>
                  </div>
